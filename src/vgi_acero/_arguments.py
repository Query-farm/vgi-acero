...
This module is that conversion layer for vgi-acero. Near-verbatim port of
vgi-polars' `_arguments.py` — this problem has nothing to do with the target
engine.
"""
...
#: value (not a per-row one), used by the scalar-function bridge (`_scalar.py`)
#: to split a declared argument schema into const vs. per-row/array fields.
